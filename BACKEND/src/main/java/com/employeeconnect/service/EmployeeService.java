...
public class EmployeeService {
	...
	public void temp(String mail) {
		Employee x = repo.findByEmail(mail).orElse(null);
		if(x==null)return;
		highrachie(x);
	}
	
	
	public void highrachie(Employee x) {
		if(x==null) {
			return;
		}
		System.out.println(x.getName());
		highrachie(x.getManager());
	}

	public ResponseEntity<List<String>> getAllMail(){
		List<Employee> all = repo.findAll();
		List<String> mails = new ArrayList<>();
		for(Employee emp:all) {
			mails.add(emp.getEmail());
		}
		return new ResponseEntity<List<String>>(mails, HttpStatus.OK); 
